// ============================================
// CHEMS IA — Servidor Backend
// ============================================
// Este servidor protege a chave do Gemini (nunca exposta ao navegador)
// e comunica com o Supabase para ler/escrever dados dos alunos.

require('dotenv').config();
const express = require('express');
const cors = require('cors');
const { GoogleGenerativeAI } = require('@google/generative-ai');
const { createClient } = require('@supabase/supabase-js');

const app = express();
app.use(cors());
app.use(express.json({ limit: '10mb' })); // limite maior por causa de fotos em base64

// ===== CONFIGURAÇÃO (lida de variáveis de ambiente — nunca escrita aqui) =====
const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
const SUPABASE_URL = process.env.SUPABASE_URL;
const SUPABASE_SERVICE_KEY = process.env.SUPABASE_SERVICE_KEY; // chave secreta, só no servidor

if (!GEMINI_API_KEY) console.error('[AVISO] GEMINI_API_KEY não definida nas variáveis de ambiente.');
if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) console.error('[AVISO] Configuração do Supabase incompleta.');

const genAI = new GoogleGenerativeAI(GEMINI_API_KEY);
const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_KEY);

const MODELO_IA = 'gemini-2.5-flash'; // 1.500 pedidos/dia gratuitos — o 2.0-flash está descontinuado (encerra Set/2026)

// ===== FUNÇÃO AUXILIAR: remove Markdown/LaTeX residual, caso a IA ainda o use por engano =====
function limparFormatacao(texto) {
  return texto
    // Nota: **negrito** é mantido de propósito — o frontend converte em <strong>
    .replace(/#{1,6}\s*/g, '')          // ### título → título
    .replace(/\$\$?(.*?)\$\$?/g, '$1')  // $formula$ ou $$formula$$ → formula
    .replace(/^[-*]\s+(?!\*)/gm, '— ')  // listas com - ou * → travessão (mas não toca em ** de negrito)
    .trim();
}

// ===== FUNÇÃO AUXILIAR: chama a IA com retry automático em caso de limite de quota =====
async function chamarIAComRetry(funcaoChamada, tentativas = 2) {
  for (let i = 0; i < tentativas; i++) {
    try {
      return await funcaoChamada();
    } catch (err) {
      const ehErroQuota = err.status === 429 || err.message?.includes('429') || err.message?.includes('quota') || err.message?.includes('Quota');
      const ultimaTentativa = i === tentativas - 1;

      if (ehErroQuota && !ultimaTentativa) {
        console.log(`[IA] Limite de quota atingido, a tentar novamente em 8s (tentativa ${i + 1}/${tentativas})...`);
        await new Promise(resolve => setTimeout(resolve, 8000));
        continue;
      }

      if (ehErroQuota) {
        const erroAmigavel = new Error('A IA está com muitos pedidos neste momento (limite gratuito atingido). Espera um minuto e tenta de novo.');
        erroAmigavel.ehQuota = true;
        throw erroAmigavel;
      }

      throw err;
    }
  }
}

// ===== INSTRUÇÃO BASE PARA A IA (personalidade do Chems IA) =====
const INSTRUCAO_SISTEMA = `Tu és a assistente de IA do Chems IA, uma plataforma educacional de Química para estudantes moçambicanos (8ª-12ª classe, currículo do MINEDH).

Regras:
- Responde sempre em português (variante de Moçambique/Portugal, nunca brasileiro).
- Sê clara, didática e encorajadora — o público são adolescentes a aprender Química.
- Usa exemplos do dia a dia quando possível.
- Se a pergunta não for sobre Química, redireciona com simpatia para o tema.
- Mantém respostas focadas e não muito longas (estudantes em telemóvel, não querem parágrafos enormes).
- Nunca inventes fórmulas, números atómicos ou dados científicos — se não tiveres certeza, diz isso.

Formatação (muito importante):
- NÃO uses ### títulos nem listas numeradas em markdown.
- NUNCA uses LaTeX nem símbolos como $...$ para fórmulas.
- Fórmulas e equações químicas: usa os números pequenos (subscrito) de verdade para os índices, assim: H₂O, CO₂, CH₄, Na⁺, OH⁻, H₂SO₄, O₂. Usa estes caracteres exactamente: ₀ ₁ ₂ ₃ ₄ ₅ ₆ ₇ ₈ ₉ e os sinais ⁺ ⁻ para carga. Nunca escrevas H2O com o número normal — escreve sempre H₂O.
- Podes usar **negrito** (com asteriscos duplos), mas só em palavras ou termos-chave realmente importantes — no máximo 1-2 por parágrafo. Não exageres, a maior parte do texto deve ficar sem negrito.
- Para listas, usa frases corridas ou travessões simples (-), nunca numeração com markdown.

Parágrafos (muito importante):
- Escreve em parágrafos curtos: no máximo 2-3 frases por parágrafo, depois muda de linha (parágrafo novo).
- Entre cada passo de um raciocínio (ex: balancear uma equação), usa um parágrafo separado — nunca tudo junto num bloco compacto.
- Imagina que a resposta vai ser lida num ecrã pequeno de telemóvel: muito espaço em branco entre ideias é melhor que texto denso.

Saudações:
- Varia a forma de começar a resposta. Não comeces sempre com "Olá" ou "Que bom que perguntaste". Às vezes vai direto ao ponto, às vezes usa uma frase diferente — mantém natural, como uma conversa real.`;

// ===== MIDDLEWARE: identifica o aluno a partir do token de autenticação =====
async function autenticarAluno(req, res, next) {
  const token = req.headers.authorization?.replace('Bearer ', '');
  if (!token) {
    console.log('[Auth] Nenhum token fornecido no cabeçalho Authorization.');
    return res.status(401).json({ erro: 'Token de autenticação não fornecido.' });
  }

  console.log('[Auth] Token recebido (primeiros 20 caracteres):', token.substring(0, 20));

  const { data, error } = await supabase.auth.getUser(token);

  if (error) {
    console.log('[Auth] Erro ao validar token:', error.message, '| Status:', error.status);
    return res.status(401).json({ erro: `Token inválido ou expirado: ${error.message}` });
  }
  if (!data.user) {
    console.log('[Auth] Token validado mas sem utilizador associado.');
    return res.status(401).json({ erro: 'Token inválido ou expirado.' });
  }

  console.log('[Auth] Autenticado com sucesso. Aluno ID:', data.user.id);
  req.alunoId = data.user.id;
  next();
}

// ===== ROTA: Listar conversas do aluno (mais recente primeiro) =====
app.get('/api/conversas', autenticarAluno, async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('conversas')
      .select('id, titulo, criada_em, actualizada_em')
      .eq('aluno_id', req.alunoId)
      .order('actualizada_em', { ascending: false });

    if (error) throw error;
    res.json({ conversas: data });
  } catch (err) {
    console.error('[Erro /api/conversas GET]', err.message);
    res.status(500).json({ erro: 'Não foi possível carregar as conversas.' });
  }
});

// ===== ROTA: Criar nova conversa =====
app.post('/api/conversas', autenticarAluno, async (req, res) => {
  try {
    const agora = new Date();
    const titulo = `Conversa de ${agora.toLocaleDateString('pt-PT', { day: '2-digit', month: '2-digit', year: 'numeric' })} às ${agora.toLocaleTimeString('pt-PT', { hour: '2-digit', minute: '2-digit' })}`;

    const { data, error } = await supabase
      .from('conversas')
      .insert({ aluno_id: req.alunoId, titulo })
      .select()
      .single();

    if (error) throw error;
    res.json({ conversa: data });
  } catch (err) {
    console.error('[Erro /api/conversas POST]', err.message);
    res.status(500).json({ erro: 'Não foi possível criar a conversa.' });
  }
});

// ===== ROTA: Listar mensagens de uma conversa =====
app.get('/api/conversas/:id/mensagens', autenticarAluno, async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('mensagens')
      .select('id, remetente, texto, criada_em')
      .eq('conversa_id', req.params.id)
      .eq('aluno_id', req.alunoId) // garante que só vê mensagens das próprias conversas
      .order('criada_em', { ascending: true });

    if (error) throw error;
    res.json({ mensagens: data });
  } catch (err) {
    console.error('[Erro /api/conversas/:id/mensagens]', err.message);
    res.status(500).json({ erro: 'Não foi possível carregar as mensagens.' });
  }
});

// ===== ROTA: Apagar conversa =====
app.delete('/api/conversas/:id', autenticarAluno, async (req, res) => {
  try {
    const { error } = await supabase
      .from('conversas')
      .delete()
      .eq('id', req.params.id)
      .eq('aluno_id', req.alunoId);

    if (error) throw error;
    res.json({ sucesso: true });
  } catch (err) {
    console.error('[Erro /api/conversas DELETE]', err.message);
    res.status(500).json({ erro: 'Não foi possível apagar a conversa.' });
  }
});

// ===== ROTA: Chat de texto com a IA (sempre grátis, sem limite) =====
// ===== FUNÇÃO AUXILIAR: gera um título curto para a conversa com base na primeira mensagem =====
async function gerarTituloConversa(conversaId, alunoId, primeiraMensagem) {
  try {
    const model = genAI.getGenerativeModel({
      model: MODELO_IA,
      systemInstruction: 'Gera um título muito curto (máximo 4-5 palavras) para uma conversa de Química que começa com a mensagem do utilizador. Responde APENAS com o título, sem aspas, sem pontuação final, sem explicação.'
    });
    const result = await model.generateContent(primeiraMensagem);
    const titulo = result.response.text().trim().replace(/^["']|["']$/g, '').slice(0, 60);

    if (titulo) {
      await supabase.from('conversas').update({ titulo }).eq('id', conversaId).eq('aluno_id', alunoId);
    }
  } catch (err) {
    console.error('[gerarTituloConversa] Falhou, mantém título por data:', err.message);
  }
}

app.post('/api/chat', autenticarAluno, async (req, res) => {
  try {
    const { mensagem, conversaId } = req.body;
    if (!mensagem || typeof mensagem !== 'string') {
      return res.status(400).json({ erro: 'Mensagem inválida.' });
    }

    // Busca o histórico desta conversa (se houver) para a IA manter o contexto
    let historico = [];
    let ehPrimeiraMensagem = false;

    if (conversaId) {
      const LIMITE_HISTORICO = 12; // últimas mensagens reenviadas à IA como contexto (poupa quota)
      const { data: mensagensAnteriores } = await supabase
        .from('mensagens')
        .select('remetente, texto')
        .eq('conversa_id', conversaId)
        .eq('aluno_id', req.alunoId)
        .order('criada_em', { ascending: true });

      if (mensagensAnteriores && mensagensAnteriores.length > 0) {
        let recentes = mensagensAnteriores.slice(-LIMITE_HISTORICO);
        // O histórico do Gemini precisa começar com 'user' — se cortámos a meio, remove a 1ª se for da IA
        if (recentes.length > 0 && recentes[0].remetente !== 'aluno') {
          recentes = recentes.slice(1);
        }
        historico = recentes.map(m => ({
          role: m.remetente === 'aluno' ? 'user' : 'model',
          parts: [{ text: m.texto }]
        }));
      } else {
        ehPrimeiraMensagem = true;
      }
    }

    const resposta = await chamarIAComRetry(async () => {
      const model = genAI.getGenerativeModel({
        model: MODELO_IA,
        systemInstruction: INSTRUCAO_SISTEMA
      });
      const chat = model.startChat({ history: historico });
      const result = await chat.sendMessage(mensagem);
      return limparFormatacao(result.response.text());
    });

    // Se foi indicada uma conversa, grava as duas mensagens (aluno + IA) e actualiza o timestamp
    if (conversaId) {
      await supabase.from('mensagens').insert([
        { conversa_id: conversaId, aluno_id: req.alunoId, remetente: 'aluno', texto: mensagem },
        { conversa_id: conversaId, aluno_id: req.alunoId, remetente: 'ia', texto: resposta }
      ]);
      await supabase
        .from('conversas')
        .update({ actualizada_em: new Date().toISOString() })
        .eq('id', conversaId)
        .eq('aluno_id', req.alunoId);

      // Gera título automático com base na primeira mensagem (silenciosamente, não bloqueia a resposta)
      if (ehPrimeiraMensagem) {
        gerarTituloConversa(conversaId, req.alunoId, mensagem).catch(err =>
          console.error('[Título automático] Erro:', err.message)
        );
      }
    }

    res.json({ resposta });
  } catch (err) {
    console.error('[Erro /api/chat]', err.message);
    if (err.ehQuota) {
      return res.status(429).json({ erro: err.message });
    }
    res.status(500).json({ erro: 'Não foi possível obter resposta da IA. Tenta novamente.' });
  }
});

// ===== ROTA: Resolver exercício por foto (limite de 10/dia, ou ilimitado se Premium) =====
app.post('/api/resolver-foto', autenticarAluno, async (req, res) => {
  try {
    const { imagemBase64, mimeType } = req.body;
    if (!imagemBase64) return res.status(400).json({ erro: 'Imagem não fornecida.' });

    // Verifica limite diário do aluno
    const { data: perfil, error: erroPerfil } = await supabase
      .from('profiles')
      .select('fotos_usadas_hoje, ultima_data_contagem, premium_ativo, premium_expira_em')
      .eq('id', req.alunoId)
      .single();

    if (erroPerfil) throw erroPerfil;

    const hoje = new Date().toISOString().split('T')[0];
    let fotosUsadas = perfil.fotos_usadas_hoje;

    // Reseta contagem se for um novo dia
    if (perfil.ultima_data_contagem !== hoje) {
      fotosUsadas = 0;
    }

    const premiumValido = perfil.premium_ativo && perfil.premium_expira_em && new Date(perfil.premium_expira_em) > new Date();

    if (!premiumValido && fotosUsadas >= 10) {
      return res.status(403).json({ erro: 'Limite diário de 10 fotos atingido. Ganha KappaCoins ou activa o Premium para continuar.', limiteAtingido: true });
    }

    const resposta = await chamarIAComRetry(async () => {
      const model = genAI.getGenerativeModel({
        model: MODELO_IA,
        systemInstruction: INSTRUCAO_SISTEMA + '\n\nO aluno enviou uma foto de um exercício de Química. Resolve-o passo a passo, explicando o raciocínio.'
      });
      const result = await model.generateContent([
        { inlineData: { data: imagemBase64, mimeType: mimeType || 'image/jpeg' } },
        { text: 'Resolve este exercício de Química passo a passo.' }
      ]);
      return limparFormatacao(result.response.text());
    });

    // Actualiza contagem
    await supabase
      .from('profiles')
      .update({ fotos_usadas_hoje: fotosUsadas + 1, ultima_data_contagem: hoje })
      .eq('id', req.alunoId);

    res.json({ resposta, fotosRestantesHoje: premiumValido ? null : (9 - fotosUsadas) });
  } catch (err) {
    console.error('[Erro /api/resolver-foto]', err.message);
    if (err.ehQuota) {
      return res.status(429).json({ erro: err.message });
    }
    res.status(500).json({ erro: 'Não foi possível analisar a imagem. Tenta novamente.' });
  }
});

// ===== ROTA: Gerar quiz (10 perguntas sobre um tema) =====
app.post('/api/gerar-quiz', autenticarAluno, async (req, res) => {
  try {
    const { tema } = req.body;

    // Verifica limite diário de quizzes (15/dia)
    const { data: perfil, error: erroPerfil } = await supabase
      .from('profiles')
      .select('quizzes_feitos_hoje, ultima_data_contagem')
      .eq('id', req.alunoId)
      .single();

    if (erroPerfil) throw erroPerfil;

    const hoje = new Date().toISOString().split('T')[0];
    let quizzesFeitos = perfil.ultima_data_contagem === hoje ? perfil.quizzes_feitos_hoje : 0;

    if (quizzesFeitos >= 15) {
      return res.status(403).json({ erro: 'Limite diário de 15 quizzes atingido. Volta amanhã!', limiteAtingido: true });
    }

    const promptTema = tema ? `sobre o tema "${tema}"` : 'sobre um tema aleatório do currículo de Química do ensino moçambicano (8ª-12ª classe)';

    const perguntas = await chamarIAComRetry(async () => {
      const model = genAI.getGenerativeModel({
        model: MODELO_IA,
        systemInstruction: `Gera exactamente 10 perguntas de Química de escolha múltipla ${promptTema}, adequadas ao currículo moçambicano.
Responde APENAS em JSON válido, sem markdown, sem texto antes ou depois, neste formato exacto:
[{"q":"texto da pergunta","opts":["op1","op2","op3","op4"],"correct":0,"exp":"explicação breve da resposta correcta"}]
O campo "correct" é o índice (0-3) da opção correcta.`
      });
      const result = await model.generateContent('Gera o quiz agora.');
      let textoResposta = result.response.text().trim();
      textoResposta = textoResposta.replace(/```json|```/g, '').trim();
      return JSON.parse(textoResposta);
    });

    // Actualiza contagem
    await supabase
      .from('profiles')
      .update({ quizzes_feitos_hoje: quizzesFeitos + 1, ultima_data_contagem: hoje })
      .eq('id', req.alunoId);

    res.json({ perguntas });
  } catch (err) {
    console.error('[Erro /api/gerar-quiz]', err.message);
    if (err.ehQuota) {
      return res.status(429).json({ erro: err.message });
    }
    res.status(500).json({ erro: 'Não foi possível gerar o quiz. Tenta novamente.' });
  }
});

// ===== ROTA: Registar resultado do quiz (dar coins + pontos de ranking) =====
app.post('/api/concluir-quiz', autenticarAluno, async (req, res) => {
  try {
    const { tema, totalPerguntas, acertos } = req.body;
    const COINS_POR_QUIZ = 25;
    const PONTOS_RANKING_POR_QUIZ = 50;

    const { data: perfil, error: erroPerfil } = await supabase
      .from('profiles')
      .select('kappa_coins, pontos_ranking_geral, pontos_ranking_mensal')
      .eq('id', req.alunoId)
      .single();

    if (erroPerfil) throw erroPerfil;

    await supabase
      .from('profiles')
      .update({
        kappa_coins: perfil.kappa_coins + COINS_POR_QUIZ,
        pontos_ranking_geral: perfil.pontos_ranking_geral + PONTOS_RANKING_POR_QUIZ,
        pontos_ranking_mensal: perfil.pontos_ranking_mensal + PONTOS_RANKING_POR_QUIZ
      })
      .eq('id', req.alunoId);

    await supabase.from('quizzes_historico').insert({
      aluno_id: req.alunoId,
      tema: tema || null,
      total_perguntas: totalPerguntas,
      acertos: acertos,
      coins_ganhos: COINS_POR_QUIZ,
      pontos_ranking_ganhos: PONTOS_RANKING_POR_QUIZ
    });

    await supabase.from('transacoes_coins').insert({
      aluno_id: req.alunoId,
      quantidade: COINS_POR_QUIZ,
      motivo: 'quiz_concluido'
    });

    res.json({ coinsGanhos: COINS_POR_QUIZ, pontosGanhos: PONTOS_RANKING_POR_QUIZ });
  } catch (err) {
    console.error('[Erro /api/concluir-quiz]', err.message);
    res.status(500).json({ erro: 'Não foi possível registar o resultado do quiz.' });
  }
});

// ===== ROTA: Buscar Ranking (geral, mensal ou por cidades) =====
app.get('/api/ranking/:tipo', autenticarAluno, async (req, res) => {
  try {
    const { tipo } = req.params; // 'geral', 'mensal', ou 'cidades'

    if (tipo === 'cidades') {
      const { data, error } = await supabase
        .from('profiles')
        .select('cidade, pontos_ranking_mensal')
        .not('cidade', 'is', null);

      if (error) throw error;

      const porCidade = {};
      data.forEach(p => {
        if (!p.cidade) return;
        porCidade[p.cidade] = (porCidade[p.cidade] || 0) + p.pontos_ranking_mensal;
      });

      const lista = Object.entries(porCidade)
        .map(([cidade, pontos]) => ({ nome: cidade, pontos }))
        .sort((a, b) => b.pontos - a.pontos);

      return res.json({ lista });
    }

    const coluna = tipo === 'mensal' ? 'pontos_ranking_mensal' : 'pontos_ranking_geral';
    const { data, error } = await supabase
      .from('profiles')
      .select(`nome, cidade, ${coluna}`)
      .order(coluna, { ascending: false })
      .limit(50);

    if (error) throw error;

    const lista = data.map(p => ({ nome: p.nome || 'Aluno', cidade: p.cidade, pontos: p[coluna] }));
    res.json({ lista });
  } catch (err) {
    console.error('[Erro /api/ranking]', err.message);
    res.status(500).json({ erro: 'Não foi possível carregar o ranking.' });
  }
});

// ===== ROTA: Saúde do servidor (para verificar se está vivo) =====
app.get('/health', (req, res) => res.json({ status: 'ok', timestamp: new Date().toISOString() }));

const PORTA = process.env.PORT || 3000;
app.listen(PORTA, () => console.log(`Chems IA backend a correr na porta ${PORTA}`));
